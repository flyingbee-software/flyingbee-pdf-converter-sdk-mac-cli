...
#  Batch-convert every PDF in a folder using OCR (scanned documents).
#
...
OUTPUT_FORMAT="docx"           # docx, pptx, xlsx, html, csv, txt.
NUM_THREADS="4"                # Number of processing threads (1 - 10).
PAGE_RANGES="all"              # Pages to convert, e.g. "all" or "1-3,5,8-10".
OCR_LANG="eng"                 # Tesseract language: eng, chi_sim, jpn, ...
LOG_FILE="conversion-ocr.log"  # Log file (relative paths resolve under the script folder).
...
echo "========================================"
echo "Flyingbee PDF Converter - macOS CLI demo (OCR)"
echo "========================================"
...
# ------------------------------------------------------------------------------
echo "========================================" > "$LOG_FILE"
echo "📄 PDF to $OUTPUT_FORMAT (OCR: $OCR_LANG) Conversion Log" >> "$LOG_FILE"
...
total=${#valid_pdfs[@]}
echo "📦 Found $total valid PDF file(s). Starting OCR ($OCR_LANG) conversion to $OUTPUT_FORMAT..."
...
for i in "${!valid_pdfs[@]}"; do
    ...
    printf '【%s】\n' "$pdf_name"
    echo "🔄 Converting with OCR ($index/$total)..."

    echo "------------------" >> "$LOG_FILE"
    printf '【%s】\n' "$pdf_name" >> "$LOG_FILE"
    echo "🔄 Converting with OCR ($index/$total)..." >> "$LOG_FILE"
    ...
    cmd=( "$TOOL_PATH" -a PDF2Files -i "$pdf_file" -r 1 -g "$OCR_LANG" -f "$OUTPUT_FORMAT" -o "$out_path" -t "$NUM_THREADS" -p "$PAGE_RANGES" )
